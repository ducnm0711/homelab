...
qbittorrent┼──► radarr ──► bazarr     (soft: logical order only)
           │
           └──► jellyfin              (soft: media appears when Radarr imports)
```
...
|---|---|
| Jellyfin, Radarr, Bazarr | Prowlarr, qBittorrent |
...
```bash
# Config directories
sudo chown -R 568:568 /mnt/SSD/config/{jellyfin,bazarr,radarr,prowlarr,qbittorrent}
sudo chmod -R 775 /mnt/SSD/config/{jellyfin,bazarr,radarr,prowlarr,qbittorrent}
